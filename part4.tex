...
\paragraph{}
React Native giao tiếp với các module native thông qua một cầu nối (bridge) bằng JavaScript.
Điều này cho phép chúng ta phát triển cho các nền tảng khác nhau nhưng lại 
nảy sinh các điểm nghẽn về hiệu năng, đặc biệt là khi animation hoặc việc tính toán quá nặng.
\paragraph{}
...
\paragraph{}
{\large \textbf{Logic nghiệp vụ dùng chung (được chia sẻ)}}
...
hiệu năng native và giao diện mang lại cảm giác quen thuộc cho người dùng.
\paragraph{}
\textbf{Hot Reload:}
\paragraph{}
...
Các thuộc tính nổi bật của nó, chẳng hạn như hot reload,
 khả năng tái sử dụng code to lớn, và một ecosystem phong phú,
...
\paragraph{}
Sự rõ ràng của codebase, tính module hoá và khả năng gỡ lỗi đơn giản
...
triển bằng JavaScript và React Framework). Tính module hóa thông qua các
...
Tính module hóa giúp việc bảo trì dễ dàng hơn vì sự thay đổi ở bất kỳ
...
tiếp được với các module native của hệ điều hành).
\paragraph{}
...
khi thực hiện gỡ lỗi giữa JavaScript và mã native. Các lỗi nằm trong lớp
...
phải triển khai UI layer riêng biệt và sử dụng ngôn ngữ native,
...
trên Widget của Flutter giúp tạo ra code sạch và thể hiện tính module cao.
\paragraph{}
...
này giúp cải thiện tính module hóa, đồng thời làm cho mã nguồn
trở nên mạch lạc và dễ bảo trì hơn. Ngoài ra, tính năng hot reload
của Flutter cho phép các lập trình viên nhanh chóng cập nhật UI,
...
\section{Tích hợp với các Native API}
\subsection{React Native}
Các module native có thể được viết bằng Java/Objective-C/Swift. Nó có điểm hạn chế là đòi hỏi nỗ
lực của lập trình viên để tạo cầu nối giữa JavaScript và mã nguồn.
\subsection{Kotlin Multiplatform}
Nó hoạt động trơn tru với Android; còn đối với iOS thì phải phát triển
bằng Swift/Objective-C.
\paragraph{}
\textbf{Hạn chế:} Để tích hợp sâu, chủ yếu phải sử dụng mã nguồn đặc thù cho từng nền tảng.
\subsection{Flutter}
Các kênh nền tảng cho phép tích hợp với mã nguồn một cách dễ dàng.
\paragraph{}
\textbf{Hạn chế:} Cần viết mã nguồn native của mỗi nền tảng cho các tính năng nâng cao.
\section{Thiết kế UI/UX của Ứng dụng}
\paragraph{}
Trong trò chơi Tic-Tac-Toe bằng Flutter, UI được thiết kế gọn gàng và
tối giản, sử dụng tốt chữ nghiêng cùng với việc sử dụng layout tốt và
đơn giản nhằm giúp chúng ta hiểu rõ hơn về lượng code tối thiểu cần
viết để đạt được mục tiêu đã định. Màn hình hiển thị tiêu đề trò chơi
“TIC-TAC-TOE” bằng chữ lớn, theo sau là phần gợi ý rằng đang đến lượt
của người chơi một.
Thiết kế có thể bao gồm một bảng phối màu đơn giản với độ tương phản cao
để dễ đọc hơn, và vị trí nút “Restart” được đặt hợp lý để sử dụng thuận
tiện. UX mang tính trực quan, với các gợi ý rõ ràng giúp dẫn dắt
người dùng theo diễn biến trò chơi. Đúng với đặc trưng của Flutter, các animation
và transition mượt mà giúp toàn bộ trải nghiệm được cải thiện hơn nữa.
Tuy nhiên, khả năng cao là app này được xây dựng sử dụng responsive design,
vì vậy nó sẽ hoạt động tốt trên các kích thước màn hình khác nhau.
Ý tưởng chủ đạo đơn giản và phù hợp với cách chơi, dễ tiếp cận với người dùng
ở mọi lứa tuổi do không bị rối mắt và tập trung vào các yếu tố thiết yếu.
Yếu tố chức năng được ưu tiên hàng đầu, giúp người dùng có thể chơi ngay
lập tức mà không mất thời gian tìm hiểu cách sử dụng.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\textwidth]{tictactoe.png}
    \caption{Giao diện trò chơi Tic-Tac-Toe bằng Flutter}
\end{figure}
\paragraph{}
Mặc dù các nguyên tắc UI trong ứng dụng Tic Tac Toe bằng React có thể
tương tự, nhưng các lựa chọn về phong cách của ứng dụng này có thể
khác biệt vì React cho phép các component của nó linh hoạt. Đầu tiên,
tiêu đề "TIC-TAC-TOE" được hiển thị một cách nổi bật, và thiết kế của
phiên bản React có thể sinh động hơn với hiệu ứng hover hoặc thậm chí một
animation nhỏ để giúp mọi thứ trở nên trực quan và có tính tương tác cao hơn.
Quản lý trạng thái của React cũng sẽ cho phép chúng ta cập nhật UI một
cách liền mạch hơn khi chơi trò chơi, vì nút “Restart” cũng được đặt ở vị
trí thuận tiện. Trong React, phần lớn thiết kế UX có khả năng sử dụng
các công nghệ web hiện đại giúp tăng cường trải nghiệm mượt mà với tốc độ
cập nhật nhanh hơn và các hiệu ứng chuyển tiếp mượt mà hơn.
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{ttt2.png}
  \caption{Giao diện trò chơi Tic-tac-toe bằng React Native}
\end{figure}
\section{Tổng kết và so sánh}
Trước khi đi sâu vào việc triển khai chi tiết hoặc lựa chọn một công nghệ
cụ thể, việc so sánh các tính năng chính và khả năng của từng framework
là rất cần thiết. Bảng dưới đây trình bày tóm tắt những khác biệt chính
giữa React Native, Kotlin Multiplatform và Flutter. Nó tập trung vào
các khía cạnh quan trọng như hiệu năng, sự thuận tiện trong quá trình
phát triển, và khả năng bảo trì mã nguồn, sự hỗ trợ từ ecosystem, và
khả năng tích hợp tốt như thế nào với các Native API. Sự so sánh này
nhằm mục đích cung cấp một cái nhìn tổng quan nhanh chóng nhưng sâu
sắc để giúp các lập trình viên và những người ra quyết định lựa chọn công
cụ phù hợp dựa trên các nhu cầu và ưu tiên của dự án.
\begin{table}[H]
    \centering
    \caption{\textbf{Tóm tắt so sánh giữa cả ba ngôn ngữ}}
    \vspace{0.3cm} % Tạo khoảng cách nhỏ giữa tên bảng và bảng
    \renewcommand{\arraystretch}{1.5} % Tăng chiều cao dòng cho thoáng
    
    % Định nghĩa bảng: X là cột tự co giãn, | là đường kẻ dọc
    \begin{tabularx}{\textwidth}{|>{\bfseries}p{3cm}|X|X|X|}
        \hline
        \rowcolor{lightgray!30} % Tô màu xám nhạt cho hàng tiêu đề
        Tiêu chí & React Native & Kotlin Multiplatform & Flutter \\ \hline
        
        % Hàng 1: Hiệu năng
        Hiệu năng & 
        Trung bình (do bridge overhead) & 
        Cao (native UI) & 
        Cao (Skia engine) \\ \hline
        
        % Hàng 2: Tốc độ phát triển
        Tốc độ phát triển & 
        Nhanh (hot reload, JavaScript) & 
        Trung bình (UI đặc thù nền tảng) & 
        Nhanh (hot reload, widgets) \\ \hline
        
        % Hàng 3: Khả năng bảo trì code
        Khả năng bảo trì code & 
        Trung bình (thách thức của JavaScript) & 
        Cao (strong typing của Kotlin) & 
        Cao (strong typing của Dart) \\ \hline
        
        % Hàng 4: Hỗ trợ hệ sinh thái
        Hỗ trợ hệ sinh thái & 
        Trưởng thành & 
        Đang phát triển & 
        Phong phú \\ \hline
        
        % Hàng 5: Tích hợp Native API
        Tích hợp Native API & 
        Yêu cầu bridging & 
        Liền mạch (Android), tốn công sức (iOS) & 
        Platform channels \\ \hline
        
    \end{tabularx}
\end{table}
\section{Khuyến nghị, đề xuất}
Có nhiều lựa chọn về các framework phát triển đa nền tảng nhằm đáp ứng nhu cầu
của dự án. React Native được khuyên dùng khi ưu tiên hàng đầu là tốc độ
phát triển, nhờ việc các kỹ sư JavaScript có thể tận dụng các phương pháp
lập trình vốn đã quen thuộc. Đối với các nhà phát triển xây dựng ứng dụng
theo hướng ưu tiên Android, Kotlin Multiplatform mang lại lợi ích to lớn,
bởi nó cho phép tái sử dụng các phần logic nghiệp vụ quan trọng sang mọi
nền tảng khác mà không gặp khó khăn. Trong trường hợp dự án yêu cầu hiệu
năng tối ưu, sự đồng nhất về thiết kế và mã nguồn hoạt động trên mọi môi
trường, thì Flutter là lựa chọn sáng giá nhờ tốc độ vượt trội của bộ máy
kết xuất và khả năng tùy biến linh hoạt các thành phần giao diện. Mỗi
framework đều sở hữu những thế mạnh riêng; hiệu quả sẽ đạt mức tối
đa khi đặc thù dự án phù hợp với các thế mạnh đó và đội ngũ phát triển
nắm vững cách khai thác chúng.
